...
const DEFAULT_COUNTRY_HINT =
  process.env.DEFAULT_COUNTRY_HINT || "República Dominicana";
...
// =====================================================
// ✅ PRO: Anti-duplicados + Lock + KeepAlive
// (NO cambia tu lógica: solo evita reintentos/carreras y mantiene despierto)
// =====================================================
const DEDUPE_TTL_SEC = Number(
  process.env.DEDUPE_TTL_SEC || 60 * 60 * 48
); // 48h
const LOCK_TTL_SEC = Number(process.env.LOCK_TTL_SEC || 15);

// Keep alive (opcional)
const KEEPALIVE_ENABLED = String(process.env.KEEPALIVE_ENABLED || "1") !== "0";
...
// Upstash REST (opcional, recomendado para persistencia real)
...
function memSet(map, key, ttlSec) {
  map.set(key, Date.now() + ttlSec * 1000);
}
function memSetNX(map, key, ttlSec) {
  const exp = map.get(key);
  if (exp && Date.now() < exp) return false;
  map.set(key, Date.now() + ttlSec * 1000);
  return true;
}
function memDel(map, key) {
  map.delete(key);
}
...
setInterval(() => {
  const now = Date.now();
  for (const [k, exp] of __dedupeMem.entries())
    if (now > exp) __dedupeMem.delete(k);
  for (const [k, exp] of __lockMem.entries())
    if (now > exp) __lockMem.delete(k);
}, 60 * 1000);
...
async function upstashSetEX(key, value, ttlSec) {
  ...
  try {
    const url = `${UPSTASH_REDIS_REST_URL}/set/${encodeURIComponent(
      key
    )}/${encodeURIComponent(value)}?EX=${ttlSec}`;
    await axios.post(
      url,
      {},
      {
...
async function upstashSetNXEX(key, value, ttlSec) {
  ...
  try {
    const url = `${UPSTASH_REDIS_REST_URL}/set/${encodeURIComponent(
      key
    )}/${encodeURIComponent(value)}?NX=1&EX=${ttlSec}`;
...
async function isDuplicateWaMessage(waMessageId) {
  ...
  const existing = memGet(__dedupeMem, key);
  if (existing) return true;
  memSet(__dedupeMem, key, DEDUPE_TTL_SEC);
  return false;
}
...
async function bothubReportMessage(payload) {
  ...
  try {
    ...
    console.error(
      "Bothub report failed:",
      (e && e.response && e.response.data) || (e && e.message) || e
    );
  }
}
...
function extractInboundMeta(msg) {
  ...
  if (msg && msg.type === "contacts")
    return { kind: "CONTACTS", count: (msg.contacts && msg.contacts.length) || 0 };
  if (msg && msg.type === "reaction")
    return {
      kind: "REACTION",
      emoji: msg && msg.reaction && msg.reaction.emoji,
      messageId: msg && msg.reaction && msg.reaction.message_id,
    };
...
function assertEnv() {
  ...
  if (!BOTHUB_WEBHOOK_URL)
    console.warn("⚠️ Missing ENV: BOTHUB_WEBHOOK_URL (Hub won't receive messages)");
  if (!BOTHUB_WEBHOOK_SECRET)
    console.warn("⚠️ Missing ENV: BOTHUB_WEBHOOK_SECRET (Hub signature will fail)");

  if (!UPSTASH_REDIS_REST_URL || !UPSTASH_REDIS_REST_TOKEN) {
    console.warn("ℹ️ DEDUPE/LOCK: usando memoria (recomendado configurar UPSTASH_REDIS_REST_URL/TOKEN)");
  }

  if (KEEPALIVE_ENABLED && !KEEPALIVE_URL) {
    console.warn("ℹ️ KEEPALIVE: configura KEEPALIVE_URL o RENDER_EXTERNAL_URL para auto-ping (opcional)");
  }
}
assertEnv();
...
function isDemoIntent(tNorm) {
  const t = tNorm || "";
  return ["demo", "reunion", "reunión", "agendar", "cita", "llamada"].some((k) =>
    t.includes(k)
  );
}
...
`,
  ...
  const text = (
    resp.data?.choices?.[0]?.message?.content ? resp.data.choices[0].message.content : ""
  ).trim();
...
function extractIncomingText(msg) {
  ...
  if (msg && msg.type === "audio" && msg.audio && msg.audio.id) return "[AUDIO]";
...
async function sendChannelsMenu(to) {
  await waSendList(to, "Canal", "Este bot es para WhatsApp ✅", "Elegir", "Canal", [
    ...
    setImmediate(async () => {
      ...
      try {
        ...
        await bothubReportMessage({
          ...
          kind:
            (inboundMeta && inboundMeta.kind) ||
            (msg && msg.type ? String(msg.type).toUpperCase() : "UNKNOWN"),
          meta: inboundMeta,
        });
        ...
        if (isHumanRequest(tNorm)) {
          ...
          await notifyAdmin(
            { ...session, notes: (session.notes || "") + " | Pidió HUMANO (keyword)" },
            from
          );
          return;
        }
...
if (KEEPALIVE_ENABLED && KEEPALIVE_URL) {
  setInterval(async () => {
    try {
      await axios.get(KEEPALIVE_URL, { timeout: 8000 });
    } catch {
      // silencioso
    }
